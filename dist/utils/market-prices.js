...
function imageForCommodity(name, category) {
...
// ─── Public API ───────────────────────────────────────────────────────────────
export async function getMarketPrices(category) {
...
export async function lookupMarketPrice(productName, category) {
    const prices = await getMarketPrices(category);
    const lower = productName.toLowerCase();
    const match = prices.find((p) => p.name.toLowerCase().includes(lower) ||
