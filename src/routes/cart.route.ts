...
router.get('/', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (!customerId) return res.status(401).json({ success: false, message: 'Not authenticated' });
...
router.delete('/items/:id', authMiddleware, async (req: Request, res: Response) => {
  ...
  } catch { res.status(500).json({ success: false, message: 'Failed to remove cart item' }); }
});
...
router.post('/checkout', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const createdOrders = await Promise.all(items.map(async (item: any) => {
      const order = await createOrder({
        customerId,
        productType: enumToCategory(item.productType as string) as VendorCategory,
      ...
      await Promise.all(vendors.map((v: any) =>
        createNotification({
          ...
          productType: enumToCategory(order.productType as string) as VendorCategory,
